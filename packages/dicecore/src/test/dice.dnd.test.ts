...
import { getInitialDefaultConfig } from '../config/default'
...
describe('已关联DND人物卡', () => {
  ...
  beforeEach(() => {
    // mock card
    const cardData = getDndCardProto()
    ...
    card = CardProvider.INSTANCE.getCardById(cardData.name) as DndCard
    // mock config // 修改为 dnd 的一些设置
    const config = getInitialDefaultConfig()
    // config.defaultRoll.expression = 'd20'
    config.rollDeciderId = 'io.paotuan.embed.dnd0'
    ConfigProvider.INSTANCE.register(MockChannelId, config)
  ...
  test('属性检定', () => {
    const roller = createDiceRoll({ command: '力量', context })
    expect(roller.output).toBe('Maca 🎲 力量 d20+3: [12]+3 = 15')
  })

  test('技能检定', () => {
    const roller = createDiceRoll({ command: '运动', context })
    expect(roller.output).toBe('Maca 🎲 运动 d20+{3}[力量]+{2}[熟练]: [12]+{3}+{2} = 17')
  })

  test('属性豁免', () => {
    const roller = createDiceRoll({ command: '力量豁免', context })
  ...
  test('st属性', () => {
    const roller = createDiceRoll({ command: 'st show 力量', context })
    expect(roller.output).toBe(`<at id="${MockUserId}"/>(铃木翼):\n力量*:17`)
  })

  test('st 展示技能', () => {
    const roller = createDiceRoll({ command: 'st show 战斧命中', context })
    expect(roller.output).toBe(`<at id="${MockUserId}"/>(铃木翼):\n战斧命中:d20+$力量调整+$熟练`)
  })

  test('st技能应展示总值和修正值', () => {
    const roller = createDiceRoll({ command: 'st show 运动', context })
    expect(roller.output).toBe(`<at id="${MockUserId}"/>(铃木翼):\n运动*:5(0)`)
  })

  test('st修改技能应重定向到修正值', () => {
    const roller = createDiceRoll({ command: 'st 运动+1', context })
    roller.applyToCard()
