...
describe('未关联人物卡', () => {
  ...
  test('基础运算', () => {
    ...
    expect(roller.output.trim()).toBe('Maca 🎲 2d10+d6+1: [2, 2]+[2]+1 = 7')
  })

  test('描述回显', () => {
    const roller = createDiceRoll('d100 侦察', context)
    expect(roller.output.trim()).toBe('Maca 🎲 侦察 d100: [2] = 2')
  })

  test('临时检定', () => {
    const roller = createDiceRoll('d100 侦察 50', context)
    expect(roller.output.trim()).toBe('Maca 🎲 侦察 d100: [2] = 2 ≤ 50 成功')
  })
  ...
  test('奖励骰别名', () => {
    ...
    expect(roller.output.trim()).toBe('Maca 🎲 2d%kl1: [2, 2d] = 2')
  })

  test('惩罚骰别名', () => {
    const roller = createDiceRoll('rp2', context)
    expect(roller.output.trim()).toBe('Maca 🎲 3d%kh1: [2d, 2d, 2] = 2')
  })

  test('奖励骰临时检定', () => {
    const roller = createDiceRoll('rb侦察50', context)
    expect(roller.output.trim()).toBe('Maca 🎲 侦察 2d%kl1: [2, 2d] = 2 ≤ 50 成功')
  })

  test('骰池别名', () => {
    const roller = createDiceRoll('ww4', context)
    expect(roller.output.trim()).toBe('Maca 🎲 4d10!>=10>=8: [2, 2, 2, 2] = 0')
  })

  test('骰池别名2', () => {
    const roller = createDiceRoll('ww4a5', context)
    expect(roller.output.trim()).toBe('Maca 🎲 4d10!>=5>=8: [2, 2, 2, 2] = 0')
  })

  test('检定别名', () => {
    const roller = createDiceRoll('rc', context)
    expect(roller.output.trim()).toBe('Maca 🎲 d%: [2] = 2')
  })

  test('暗骰 flag', () => {
    const roller = createDiceRoll('rh心理学', context)
    expect(roller.output.trim()).toBe('Maca 🎲 心理学 d100: [2] = 2')
  })

  test('flag 组合', () => {
    const roller = createDiceRoll('rqx3 手枪连射', context)
    expect(roller.output.trim()).toBe('Maca 🎲 手枪连射\nd100 = 2 \nd100 = 2 \nd100 = 2')
  })

  test('flag 组合2', () => {
    const roller = createDiceRoll('rb2qh 组合', context)
    expect(roller.output.trim()).toBe('Maca 🎲 组合 3d%kl1 = 2')
  })

  test('inline', () => {
    const roller = createDiceRoll('d[[d100]]', context)
    expect(roller.output.trim()).toBe('Maca 🎲\n先是 🎲 d100: [2] = 2 \n最后 🎲 d2: [2] = 2')
  })
...
describe('已关联人物卡', () => {
  ...
  test('检定', () => {
    const roller = createDiceRoll('d100 侦察', context)
    expect(roller.output.trim()).toBe('Maca 🎲 侦察 d100: [2] = 2 ≤ 40 成功')
  })
  ...
  test('不存在技能仅回显', () => {
    ...
    expect(roller.output.trim()).toBe('Maca 🎲 不存在技能 d100: [2] = 2')
  })

  test('人物卡引用', () => {
    const roller = createDiceRoll('3d100<=$力量', context)
    expect(roller.output.trim()).toBe('Maca 🎲 3d100<=60: [2*, 2*, 2*] = 3')
  })

  test('人物卡引用使用大括号', () => {
    const roller = createDiceRoll('${力量}+${理智}', context)
    expect(roller.output.trim()).toBe('Maca 🎲 60+30: 60+30 = 90')
  })

  test('直接引用表达式', () => {
    const roller = createDiceRoll('徒手格斗', context)
    expect(roller.output.trim()).toBe('Maca 🎲 徒手格斗\n先是 🎲 db 0: 0 = 0 \n最后 🎲 1d3+0: [2]+0 = 2')
  })

  test('描述不应解析为表达式', () => {
    const roller = createDiceRoll('d% 徒手格斗', context)
    expect(roller.output.trim()).toBe('Maca 🎲 徒手格斗 d%: [2] = 2')
  })
